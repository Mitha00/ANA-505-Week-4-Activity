...
#TASK: Write the function to save the two columns as a new dataset and give it a name
HairAndSex <-hair_eye_colour[,c("Hair","Sex")]

#Let's get rid of the Sex column in the new dataset created above
#TASK: Write the function that deselects the sex column
#(hint: use the 'select' function to not select a -column)
is.data.frame(HairAndSex)
select(HairAndSex, -c(Sex))
...
HairAndGender <- rename(HairAndSex, gender=Sex)

#Let's 'filter' just the males from our dataset
#TASK: Write the function that includes only rows that are 'male'
filter(HairAndGender, gender == "Male")
...
#After you run it, write the total here: 32 observations with 5 variables with a total frequency of 592
total=mutate(hair_eye_colour, total=cumsum(Freq))

#Since we have a males dataset, let's make a females dataset
#TASK: Write the function that includes only rows that are 'female'
filter(HairAndGender, gender == "Female")

#And now let's join the males and females
#TASK: Write the function that joins the male and female rows 
#(hint: try using 'union' or 'bind_rows')
HairAndFemale <-filter(HairAndGender, gender == "Female")
